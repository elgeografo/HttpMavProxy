...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../mavlink/mavlink-master01'))
# fin LIM
...
class MavLinkRpycClientService(rpyc.Service):
    ...
    def exposed_sendCommand(self, comando):
        print "COMANDO : " + comando
        mpstate.input_queue.put(comando)
        print "imprimiendo comando desde MavProxy:" + comando

    def exposed_kill(self):
        threadMavLinkCLient.stop()
        sys.exit()
# FIN LIM
...
def process_master(m):
    ...
    if msgs:
        for msg in msgs:
            ...
            # INICIO LIM
            mtype = msg.get_type() # LIM
            #print "TIPOM " + mtype
            try: # see https://pixhawk.ethz.ch/mavlink/

                if mtype is 'ATTITUDE': # https://pixhawk.ethz.ch/mavlink/#ATTITUDE
                    c.root.imprime(msg.to_json())
                if mtype is 'GLOBAL_POSITION_INT': # https://pixhawk.ethz.ch/mavlink/#GLOBAL_POSITION_INT
                    c.root.imprime(msg.to_json())
                if mtype is 'GPS_STATUS': # https://pixhawk.ethz.ch/mavlink/#GPS_STATUS
                    c.root.imprime(msg.to_json())
                if mtype is 'HEARTBEAT': # https://pixhawk.ethz.ch/mavlink/#GPS_STATUS
                    c.root.imprime(msg.to_json())
                if mtype is 'SYS_STATUS':
                    c.root.imprime(msg.to_json())
                if mtype is 'GPS_RAW_INT':
                    c.root.imprime(msg.to_json())
                """
                if mtype is 'VFR_HUD':
                    print 'VFR_HUD ' +  msg.to_json()
                if mtype is 'AHRS':
                    print 'AHRS ' +  msg.to_json()
                if mtype is 'HWSTATUS':
                    print 'HWSTATUS ' +  msg.to_json()
                if mtype is 'SYSTEM_TIME':
                    print 'SYSTEM_TIME ' +  msg.to_json()
                if mtype is 'RAW_IMU':
                    print 'RAW_IMU ' +  msg.to_json()
                if mtype is 'SCALED_PRESSURE':
                    print 'SCALED_PRESSURE ' +  msg.to_json()
                if mtype is 'SENSOR_OFFSETS':
                    print 'SENSOR_OFFSETS ' +  msg.to_json()
                """
                """
                if mtype is 'HEARTBEAT':
                    print 'HEARTBEAT ' +  msg.to_json()
                if mtype is 'SYS_STATUS':
                    print 'SYS_STATUS ' +  msg.to_json()
                if mtype is 'MEMINFO':
                    print 'MEMINFO ' +  msg.to_json()
                if mtype is 'HWSTATUS':
                    print 'HWSTATUS ' +  msg.to_json()
                if mtype is 'MISSION_CURRENT':
                    print 'MISSION_CURRENT ' +  msg.to_json()
                if mtype is 'NAV_CONTROLLER_OUTPUT':
                    print 'NAV_CONTROLLER_OUTPUT ' +  msg.to_json()
                """
            except:
                print "(LIM) ERROR from MavProxy:  en el envio del paquete desde mavLink"
            # FIN LIM
...
if __name__ == '__main__':
    ...
    #inicio LIM  *********************************
    try:
        c = rpyc.connect("localhost", 18861) #To send messsges to the web server
    except:
        print "(LIM) ERROR from Mavproxy: impossible to connect through rpyc Mav -> Web  "
        sys.exit() # frozamos la salida. no puedo usar return ya que esta fuera de una funcion
    try:
        threadMavLinkCLient = mavLinkServiceClientThread() #LIM
        threadMavLinkCLient.start()#  to recive messages from the web server
    except:
        print "(LIM) ERROR from MavProxy: Impossible to start the thread to recive command from Web"
        sys.exit()
    # fin LIM **********************************
